...
<dialog id="notifModal" class="modal">
    <div class="modal-box" x-data="{
        notifs: [1,2,3,4,12,21,14],
        selectedNotifType: 'unread',
        page: 1,
        resetNotifs(notifType) {

            this.selectedNotifType = notifType;
            this.notifs = [12,414,235,1241,1214];
            this.page = 0;
            console.log('refreshing ' + notifType + ' data');
        },
        loadMoreNotifs(notifType, page) {
            {{-- this.notifs = []; --}}
            this.notifs.push(12,414,235,1241,1214);
            this.page++;
        ...
        {{-- <div class="card-body"> --}}
            ...
            <div class="flex justify-between">
                <div role="tablist" class="tabs tabs-bordered">
                    <a role="tab" class="tab tab-active" :class="{ 'tab-active' : selectedNotifType == 'unread'}" @click="resetNotifs('unread')">Terbaru <div class="ml-2 badge badge-sm badge-secondary">2</div></a>
                    <a role="tab" class="tab" :class="{ 'tab-active' : selectedNotifType == 'read'}" @click="resetNotifs('read')">Dah Baca</a>
                </div>
            ...
            <div class="flex flex-col gap-2 border-b max-h-[30rem] overflow-y-auto overflow-x-hidden">
                ...
                <div class="h-40" x-intersect="loadMoreNotifs(selectedNotifType, page)"></div>
            </div>
